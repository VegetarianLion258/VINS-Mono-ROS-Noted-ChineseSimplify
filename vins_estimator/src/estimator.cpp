...
void Estimator::processImage(
  ...
  ROS_DEBUG("Adding feature points %lu", image.size());
	LOG(ERROR) << "frame_count: " << frame_count;
	LOG(ERROR) << "all_frame_count: " << all_image_frame.size();
  // Step 1
  // 将特征点信息加到f_manager这个特征点管理器中，同时进行是否关键帧的检查
  ...
  if (solver_flag == INITIAL) {
    if (frame_count == WINDOW_SIZE)  // 有足够的帧数
    {
      ...
      if (result) {
        ...
        slideWindow();
        // Step 6： 移除无效地图点
        f_manager.removeFailures();  // 移除无效地图点(不能被三角化的点)
...
bool Estimator::initialStructure() {
  ...
  for (int i = 0; frame_it != all_image_frame.end(); frame_it++) {
    ...
    if ((frame_it->first) == Headers[i].stamp.toSec()) {
      ...
      frame_it->second.R =
          Q[i].toRotationMatrix() * RIC[0].transpose();  // 得到Rwi
...
bool Estimator::visualInitialAlign() {
  ...
  VectorXd dep =
      f_manager.getDepthVector();  // 根据有效特征点数初始化这个动态向量
  for (int i = 0; i < dep.size(); i++) dep[i] = -1;  // 深度预设都是-1
...
/**
 * @brief
 * 由于ceres的参数块都是double数组，因此这里把参数块从eigen的表示转成double数组
 *
 */
...
void Estimator::optimization() {
      ...
      if (ESTIMATE_TD) {
        ...
        */
        ...
        ProjectionFactor *f = new ProjectionFactor(
            pts_i, pts_j);  // 构造函数就是同一个特征点在不同帧的观测
  ...
  if (relocalization_info) {
    ...
    problem.AddParameterBlock(relo_Pose, SIZE_POSE,
                              local_parameterization);  // 需要优化的回环帧位姿
  ...
  for (int i = 0; i < WINDOW_SIZE; i++) {
    if (relo_frame_stamp == Headers[i].stamp.toSec()) {
      ...
      for (int j = 0; j < SIZE_POSE; j++)
        relo_Pose[j] =
            para_Pose[i][j];  // 借助VIO优化回环帧位姿，初值先设为当前帧位姿
    }
  }
}
